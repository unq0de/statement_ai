...
| 🗑️ **Right to erasure** | [`DELETE /api/auth/account/`]({{ '/api-reference/account/' | relative_url }}#delete-my-account) permanently removes the account, statements, transactions & PDFs |
| 📤 **Right of access / portability** | [`GET /api/auth/export/`]({{ '/api-reference/account/' | relative_url }}#export-my-data) returns all personal data as JSON |
| 🔒 **Access control** | PDFs are never publicly reachable, only via the [authenticated download endpoint]({{ '/api-reference/statements/' | relative_url }}#download-the-original-pdf) |
...
Next: the [API Reference]({{ '/api-reference/' | relative_url }}) documents the erasure and export endpoints referenced above in full detail.
